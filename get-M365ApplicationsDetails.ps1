...
function Get-ServicePrincipalRoleById {
    ...
    if (!$OAuthScopes[$resID]) {
        # Use Get-MgServicePrincipal to retrieve the service principal data
        $servicePrincipal = Get-MgServicePrincipal -Filter "appId eq '$resID'"
        $OAuthScopes[$resID] = $servicePrincipal
    }
...
foreach ($App in $Apps) {
    ...
    $i++;$objPermissions = [PSCustomObject][ordered]@{
        ...
        "Key credentials expiry date" = (&{if ($app.keyCredentials) {($app.keyCredentials.endDateTime | sort -Descending | select -First 1).ToString() } else {""}})
        "Password credentials" = (&{if ($app.passwordCredentials) {parse-Credential $app.passwordCredentials} else {""}})
        "Password credentials expiry date" = (&{if ($app.passwordCredentials) {($app.passwordCredentials.endDateTime | sort -Descending | select -First 1).ToString() } else {""}})
    }
...
#Output the result
$output | select * -ExcludeProperty Number 